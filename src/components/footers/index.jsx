function Footer() {
  return (
    <>
      <footer className="absolute  dark:bg-slate-700 bg-transparent w-full">
        <div className="text-teal-700 body-font">
          <div className="">
            <div className="container mx-auto py-4 px-5 flex flex-wrap-reverse flex-col sm:flex-row">
              <p className="text-white dark:text-gray-50 text-sm text-center sm:text-left">
                © Copyright - 2023 R. Juanda Patra —
                <a
                  href=""
                  className="text-teal-600 dark:text-gray-50 ml-1 "
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  @juandap_106
                </a>
              </p>
              <span className="sm:ml-auto sm:mt-0 mt-2 sm:w-auto w-full sm:text-left text-center text-white text-sm">
                <div className="flex justify-center dark:text-gray-50">
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    stroke-width="0"
                    viewBox="0 0 448 512"
                    className="cursor-pointer mx-4 text-2xl bg-grey-50"
                    height="24"
                    width="24"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M416 32H31.9C14.3 32 0 46.5 0 64.3v383.4C0 465.5 14.3 480 31.9 480H416c17.6 0 32-14.5 32-32.3V64.3c0-17.8-14.4-32.3-32-32.3zM135.4 416H69V202.2h66.5V416zm-33.2-243c-21.3 0-38.5-17.3-38.5-38.5S80.9 96 102.2 96c21.2 0 38.5 17.3 38.5 38.5 0 21.3-17.2 38.5-38.5 38.5zm282.1 243h-66.4V312c0-24.8-.5-56.7-34.5-56.7-34.6 0-39.9 27-39.9 54.9V416h-66.4V202.2h63.7v29.2h.9c8.9-16.8 30.6-34.5 62.9-34.5 67.2 0 79.7 44.3 79.7 101.9V416z"></path>
                  </svg>
                </div>
              </span>
            </div>
          </div>
        </div>
      </footer>
    </>
  );
}

export default Footer;
